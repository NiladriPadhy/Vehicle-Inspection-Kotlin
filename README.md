...
- **Reports** — an in-app report screen (Inspected Vehicle Details, an "At a glance" rating gauge,
  and a category-wise Inspection summary), plus two exports: a **branded, multi-section PDF**
  (cover, contents, at-a-glance gauge, category summaries, parameter tables, photo gallery, and
  damage-evidence pages) rendered offline from an HTML/CSS template via WebView, and a structured
  **JSON** report. The vendor `VENDOR_ID` is shown as the report **Company Name**. Reports
  regenerate on entry, so edits are always reflected.
...
| Vehicle details, at-a-glance gauge, category summary | Generate the report as a branded PDF or JSON |
...
are filled in, the app runs in **offline baseline mode** (bundled questionnaire, local-only
accounts) — everything still works, just without RTDB sync.
...
| `VENDOR_ID` | Vendor identity; formatted and printed as **Company Name** on reports (`default` = hidden) |
